...
对管道的读写操作与go中的管道读写类似，只是参数位置有所不同，如下面的读写管道语句：
...
上述代码对数据量进行了判断，当排序数组的长度小于等于`10000`时使用普通排序，这是为了避免频繁创建线程带来过大开销。我们的测试数据量为`1000000`，使用python生成随机数据然后输入给程序，输入以下命令即可运行性能测试：

```bash
$ python .\testcase\perf_gen_input.py | go run .\testcase\perf.go
```
