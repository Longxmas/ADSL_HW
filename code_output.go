...
var i int
for _i := 0; _i < len(index); _i++ { go parallel_1(index[_i], ret[_i]) }
for _, i = range index {
<- ret[i - 1]
}
for _i := 0; _i < len(ret); _i++ { go parallel_2(ret[_i]) }
for _, i = range index {
<- ret[i - 1]
}
fmt.Printf("value1: %d\n", value1)
...
func parallel_1 (x int, r chan bool) {
if x == 1 {
var send int = 123
var receive bool
receive = <- p31
